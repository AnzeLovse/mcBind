...
def run(protein, data_set):
    ...
    training_data = load_data(
        "datasets/clip/%s/30000/training_sample_%s" % (protein, data_set), go=False, kmer=False
    )
    ...
    test_data = load_data(
        "datasets/clip/%s/30000/test_sample_%s" % (protein, data_set), go=False, kmer=False
    )
...
if __name__ == "__main__":
    ...
    for dataset in range(3):
        for protein in protein_list:
            score, predictions = run(protein, dataset)
            with open(
